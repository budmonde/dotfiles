---
description: Audit summary-provider for intra-wiki document quality and WMP discipline. Plans a per-doc fan-out, dispatches wiki-auditor/doc-reader instances with per-doc evaluation prompts, and rolls up. Read-only.
mode: subagent
permission:
  edit: deny
  write: deny
...
Your charter is intra-wiki document quality and WMP discipline.
...
- The `workspace-memory-protocol` skill for global WMP rules
- The project-specific overrides documented in `<docs_root>/index.md` (or `<docs_root>/AGENTS.md`)
- The relevant workspace-vendored operational skills in `<docs_root>/skills/`
  (for example, commit conventions, scope-visibility hygiene, and audit MO)
...
findings originate from inspecting the current wiki against `mission.md`, the WMP, and the conventions.
...
- Foundational docs: `mission.md`, `architecture.md`, `roadmap.md`, `index.md`, `todo.md`
  (or `AGENTS.md` instead of `index.md` for protocol-default projects).
- Each workspace-vendored operational skill in `skills/`.
...
3. Wave 3: `index.md`, each operational skill in `skills/` (depend on architecture and roadmap).
...
- Design-principle coherence: do the principles align with the operational MO described by the relevant skills?
...
### Each workspace-vendored operational skill

Check:

- Source ownership: the source lives in `<docs_root>/skills/<name>/`, not in a checked-in runtime discovery directory.
- Trigger description: the `description` field accurately names when the skill applies.
- Reference currency: linked references resolve and express current operational practice.
- Installation discipline: the index identifies the Skilltap vendor source and the initialization path.
- No strikethrough; present-tense operational guidance.
...
- No enumeration of individual artifacts (per the WMP anti-pattern list).
...
- **No-strikethrough across foundational docs and skills**: run
  `rg -n "~~" <docs_root>/mission.md <docs_root>/architecture.md <docs_root>/roadmap.md <docs_root>/index.md <docs_root>/todo.md <docs_root>/skills/`.
...
- **Path-form references to archived files**: enumerate archive entries (`ls <docs_root>/archive/`).
  ...
  Commit messages are exempt per the workspace's conventions skill but are not searched here anyway.
- **Provenance-discipline scope gaps**: read the workspace's conventions skill to extract the listed governed files.
...
- **high**: a WMP rule is actively violated (strikethrough in a foundational doc, checked-off todo, structural-map disagreement with the filesystem).
- **medium**: the WMP is being drifted from but not violated outright (an unflushed todo whose anchor has resolved, a stale path-form reference).
- **low**: a watch item (a dormant Stub, a long-gap research artifact).

Root-cause guidance:

- **operator-slip**: the WMP is correctly documented; the operator drifted from it.
...
- **runbook-flaw**: the WMP rule as written does not prevent this drift.
