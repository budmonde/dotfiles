...
When invoking `git` (or any other tool) against a directory other than the current working directory, prefer the shell tool's `workdir` parameter over composing the redirect into the command string itself.
...
The `workdir` parameter is a shell-tool input that does not enter the matched command string, so it bypasses the matcher quirk entirely.
...
Use the runtime's task-list tool when available (`todowrite` in OpenCode, `update_plan` in Codex) to maintain session-local progress visibility.
`AGENTS/todo.md` is the canonical cross-session task list; the runtime task list is the ephemeral intra-session overlay.
